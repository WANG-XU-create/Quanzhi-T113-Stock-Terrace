...
typedef enum
{
    ...
    VIDEO_PIXEL_FORMAT_YUV_PLANER_411      = 22,
...
typedef struct
{
	...
	int          is_10bit_flag;
	int          lbc_mod;//1:1.5x; 2:2x; 3:2.5x;
	unsigned int is_lossy; //lossy compression or not
	unsigned int rc_en;    //compact storage or not
	VoutRect rect;
}SrcInfo;
...
typedef struct
{
	...
	int (*setRectFake)(void *hdl, VoutRect *rect);
	...
	int (*setCache)(void *hdl,int enable);
	int (*getChannel)(void *hdl);
	int (*getHlayer)(void *hdl);
	int disp_fd;
	int enable;
	int disp_screen;
	int interBufSet[2];
