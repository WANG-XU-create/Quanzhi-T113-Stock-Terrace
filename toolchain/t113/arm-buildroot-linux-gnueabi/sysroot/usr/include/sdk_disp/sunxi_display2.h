...
#ifdef KERNEL_5_15
struct disp_tfbc_info {
    unsigned int lossy_rate;
    unsigned int is_gpu_header;/* start with IMG*/
};
#endif
struct disp_fb_info {
 ...
 *            unit: pixel
 * @tfbd_en: indicate if enable tfbd function
...
#ifdef KERNEL_5_15
/* disp_color_matrix - pixel color transform matrix
 *
...
struct disp_tv_func {
#ifdef KERNEL_5_15
    int (*tv_enable_early)(u32 sel);
#endif
    int (*tv_enable)(u32 sel);
#ifdef KERNEL_5_15
    int (*tv_sw_enable_early)(u32 sel);
    int (*tv_sw_enable)(u32 sel);
#endif
...
enum tag_DISP_CMD {
    ...
    DISP_NODE_LCD_MESSAGE_REQUEST = 0x19,
    DISP_RELOAD_LCD = 0x20,
    DISP_GET_HW_INFO = 0x21,
    DISP_GET_BOOT_CONFIG = 0x22,
    DISP_DEVICE_GET_FPS = 0x23,
    ...
    DISP_DEVICE_SET_COLOR_MATRIX = 0x4c,
    ...
    /* ----edp---- */
    DISP_DP_SUPPORT_MODE = 0x120,
    DISP_EDP_SET_BRIGHTNESS = 0x121,
    DISP_EDP_GET_BRIGHTNESS = 0x122,
...
/* struct disp_channel_hw_feat - channel hardware info struct
 *
 * @type: 1:video channel 0: ui channel
 * @bool: 1:support 0:not support
 *
 */
struct disp_channel_hw_info {
    bool type;
    bool vep;
    bool fbd;
    bool atw;
...
/* struct disp_dispout_hw_info - display out hardware info struct
 *
 * @bool: 1:support 0:not support
 *
 */
struct disp_dispout_hw_info {
    bool smbl;
    bool deband;
    bool gamma;
    bool dither;
...
/* struct disp_de_hw_info - display engine hardware info struct
 *
 * @disp_out: display out struct
 * @chn_num: number of channels of the display engine
 * @chn: channel struct array
 *
 * note: define union for compatible with 32bit programs
 *
 */
struct disp_de_hw_info {
    struct disp_dispout_hw_info disp_out;
    unsigned int chn_num;
    union {
        long long reserve0;
        struct disp_channel_hw_info *chn;
    };
};

/* struct disp_hw_info - display hardware info struct
 *
 * @disp_num: number of display engine
 * @disp: display engine struct array
 *
 * note: define union for compatible with 32bit programs
 *
 */
struct disp_hw_info {
    ...
    union {
        long long reserve0;
        struct disp_de_hw_info *disp;
    };
};

/* struct disp_boot_config - display boot config
 *
 * @logic_id: 0:primary display, 1:extend display
 * @de_id: display engine hardware index
 * @type: output device type
 * @fb: android ui framebuffer size
 * @dpi: dots per inch x/y
 *
 */
struct disp_boot_config {
    unsigned int logic_id;
    unsigned int de_id;
    char type[16];
    struct disp_rectsz fb;
    unsigned int dpix;
    unsigned int dpiy;
};

/* struct disp_boot_config_list - display boot config list
 *
 * @num: list num
 * @cfgs: pointer config list
 *
 */
struct disp_boot_config_list {
    unsigned int num;
    struct disp_boot_config cfgs[0];
};
#endif
