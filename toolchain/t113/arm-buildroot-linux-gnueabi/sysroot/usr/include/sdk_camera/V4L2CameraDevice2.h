...
#include <sdk_misc/Thread.h>
#include <sdk_memory/sunxiMemInterface.h>
#include <sdk_camera/type_camera.h>
#include <sdk_camera/ui/Rect.h>
#include <sdk_camera/OSAL_Queue.h>
#include <sdk_camera/Libve_Decoder2.h>
...
#endif
#ifdef WATERMARK
#include <sdk_camera/water_mark_interface.h>
#endif

#define NB_BUFFER 6
#define STATISICS_CNT    60

// fix encode or de takes too many times
#define USE_G2D_COPY 0
// #define USE_MP_CONVERT 1
...
#if USE_MP_CONVERT
#define G2D_BUFFER_SIZE (1280 * 720 * 3 / 2)
#endif

#define MAX_PICTURE_SIZE (1280*720*3/2)

//SONIX_CAMERA_H264 USED FOR 0 OR 1
#define SONIX_CAMERA_H264 1
#define CVBS_DEVICE_NUM_START 4

//#define NPU_DETECT_FPS_DEBUG
...
protected:
    ...
    class DoNpuWorkThread: public Thread
    {
        V4L2CameraDevice *mV4l2CameraDevice;
        bool mRequestExit;
    public:
        DoNpuWorkThread(V4L2CameraDevice *dev): Thread(false),
            mV4l2CameraDevice(dev), mRequestExit(false)
        {
        } void startThread()
        {
            run("DoNpuWorkThread", PRIORITY_URGENT_DISPLAY);
        }
        void stopThread()
        {
            mRequestExit = true;
        ...
        virtual bool threadLoop()
        {
            ...
            return mV4l2CameraDevice->npuThread();
        }
    };
    #endif
public:
    ...
    int orl_fmt_set(int num, void *res);
    int npuDetectWork(V4L2BUF_t *pbuf);
    int npuAwnnDetectWork(void);
    #endif

    #if defined NPU_DETECT_ENABLE && defined NPU_DETECT_FACE_ENBALE
    int npuDetectFaceWork(V4L2BUF_t *pbuf);
    #endif
    ...
    void releasePreviewFrame(int index, bool force = false);
    ...
    void setTakePictureState(TAKE_PICTURE_STATE state);
...
private:
    ...
    typedef struct bufferManagerQ_t {
        int dma_fd[NB_BUFFER];
        uint8_t *buf_vir_addr[NB_BUFFER];
    ...
    #ifdef NPU_DETECT_ENABLE
    OSAL_QUEUE mQueueBufferNpuDetect;
    #endif
    ...
    #ifdef NPU_DETECT_ENABLE
    sp < DoNpuWorkThread > mNpuWorkThread;
    pthread_mutex_t mNpuWorkMutex;
    pthread_cond_t mNpuWorkCond;
    #endif
