...
#include <sdk_camera/camera_common.h>
...
typedef char *(*camera_request_memory)(int fd, size_t buf_size, unsigned int num_bufs,
                                       void *user);
...
#ifdef ADAS_ENABLE
typedef void (*camera_adas_data_callback)(int32_t msg_type,
                                          const char *data, unsigned int index,
                                          void *metadata, void *user);
#endif
...
typedef struct preview_stream_ops {
    ...
    int (*lock_buffer)(struct preview_stream_ops *w,
                       buffer_handle_t *buffer);
    int (*set_buffers_transform)(struct preview_stream_ops *w,
                                 int transform);
...
typedef struct camera_device_ops {
    ...
    void (*set_callbacks)(struct camera_device *,
                          ...
                          void *user);
    #ifdef ADAS
    void (*gen_set_callbacks)(struct camera_device *dev, general_camera_data_callback gen_data_cb);
    int (*set_adasCarSpeed)(struct camera_device *dev, float speed);
    int (*set_laneLineOffsetSensity)(struct camera_device *dev, int level);
    #endif

    #ifdef ADAS_ENABLE
    void (*set_adas_callbacks)(struct camera_device *dev, camera_adas_data_callback adas_data_cb);
    int (*set_adas_parameters)(struct camera_device *dev, int key, int value);
    #endif
    ...
    */
    ...
    void (*set_analog_input_color)(struct camera_device *, int brightness, int contrast, int saturation);
    /**
    ...
    //for flip setting
    int (*set_CameraFlipStatus)(struct camera_device *, int flipEnable);
    #ifdef WATERMARK
    //for watermark setting
    int (*set_waterMarkMultiple)(struct camera_device *, const char *mWaterMark, int dispMode);
    #endif
    int (*set_previewparam)(struct camera_device *, struct view_info pvi, struct src_info  psi, bool tvout, int tvch);
