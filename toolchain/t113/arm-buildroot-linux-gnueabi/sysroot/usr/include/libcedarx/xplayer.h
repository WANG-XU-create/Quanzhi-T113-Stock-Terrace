...
enum MediaErrorType
{
    ...
    AW_MEDIA_ERROR_AUDIO_UNSUPPORTED = -100,
};
...
typedef enum AwApplicationType {
    ...
    APP_CMCC_WASU,
    APP_CMCC_LOCAL,
    APP_TVD_FILE_MANAGER,
} AwApplicationType;

#if TINA_LINUX_SUPPORT
enum XPlayerControlType
{
    XPLAYER_CMD_GET_AUDIO_TRACK = 0,
    XPLAYER_CMD_GET_AUDIO_BALANCE,
    XPLAYER_CMD_GET_VIDEO_DISP_FRAME_RATE,
    XPLAYER_CMD_SET_AUDIO_BALANCE,
    XPLAYER_CMD_SET_ROTATE_DEGREE,
    XPLAYER_CMD_SET_HOLD_LAST_PICTURE,
    XPLAYER_CMD_SET_SCALEDOWN_RATIO,
};
#endif
...
int XPlayerSetFloatSpeed(XPlayer* p, float nSpeed);
...
int XPlayerSetVideoPlaybackSpeed(XPlayer* p,float speed);
...
void XPlayerSetDiDemo(XPlayer* p, int enable, int left, int top, int right, int bottom);

void XPlayerSetDiTnr(XPlayer* p, int enable, int mode, int strength);

void XPlayerSetDiFmd(XPlayer* p, int enable);
#endif
#if TINA_LINUX_SUPPORT
int XPlayerControl(XPlayer* p, int cmd, void* param1, void* param2);
#endif
