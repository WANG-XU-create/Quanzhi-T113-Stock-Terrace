...
enum EPLAYERNOTIFY {
    ...
    PLAYBACK_RESET_BUFFER_FINISHED,

    PLAYBACK_NOTIFY_SCALEDOWN_FLAG,
};
...
{
    AW_FLAG_NONE  = 0x00000000,
    AW_FLAG_NOT_USE_AFBC = 0x00000001,
    //To add more flag to config video here.
};
...
enum EPLAYERINFO
{
    PLAYER_CMD_GET_AUDIO_TRACK = 0,
    PLAYER_CMD_GET_VIDEO_DISP_FRAME_RATE,
    PLAYER_CMD_SET_VIDEO_LBC_LOSSY_MOD,
};
#endif
...
//** Play Control APIs.
//**
int PlayerStreamIndexValid(Player*         pl,
...
void PlayerSetFloatSpeed(Player* pl, float nSpeed); //playback with float nSpeed times.
//********************************  END  ***********************************//
...
void PlayerSetDiDemo(Player* p, int enable, int left, int top, int right, int bottom);

void PlayerSetDiTnr(Player* p, int enable, int mode, int strength);

void PlayerSetDiFmd(Player* p, int enable);
#endif

void PlayerSetFlag(Player* pl, int flag);

#if TINA_LINUX_SUPPORT
int PlayerControl(Player* pl, int cmd, void* param);
#endif
//********************************  END  ***********************************//
